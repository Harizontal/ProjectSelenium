...
namespace SeleniumInitialize_Tests.IWebElement
{
    public class IWebElement7 : TestBaseSetUp
    {
        ...
        public void WaitStatusElements()
        {
            ...
            Assert.That(Builder.RgbaToHex(btnGosusslugi.GetCssValue("background-color")), Is.EqualTo("#F26126"));
            Actions actions = new Actions(driver);
            actions.MoveToElement(btnGosusslugi).Perform();
            Thread.Sleep(100);
            Assert.That(Builder.RgbaToHex(btnGosusslugi.GetCssValue("background-color")), Is.EqualTo("#D44921"));
        }
        [Test]
        public void WaitStatusElements2()
        {
            ...
            var input = driver.FindElement(By.XPath("//rui-range-slider[@id='amountPledge']/rui-form-field-wrapper/rui-form-field/div/input"));
            Actions actions = new Actions(driver);
